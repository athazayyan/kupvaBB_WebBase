...
export default function KupvaSekitarPage() {
  ...
  useEffect(() => {
    async function fetchKupvaData() {
      try {
        ...
        const { data: kupvaProfiles, error: profileError } = await supabase
          .from("profile")
          .select("*")
          .eq("role", "kupva")
          .not("latitude", "eq", 0)
        ...
        console.log("Fetched KUPVA profiles:", kupvaProfiles);

        if (!kupvaProfiles || kupvaProfiles.length === 0) {
          console.log("No KUPVA profiles found with valid coordinates");
          setKupvaData([]);
          setLoading(false);
          return;
        }
        ...
        const kupvaWithRates = await Promise.all(
          kupvaProfiles.map(async (kupvaProfile) => {
            try {
              ...
              console.log(`Rates for ${kupvaProfile.nama}:`, rates);

              // Format the rates data
              const formattedRates = (rates || []).map((rate) => ({
                ...rate,
                mata_uang: rate.mata_uang as unknown as Currency,
              }));
              ...
              console.error(
                `Error processing KUPVA ${kupvaProfile.nama}:`,
                error
              );
              return {
                ...kupvaProfile,
        ...
        console.log("KUPVA with rates:", kupvaWithRates);
        setKupvaData(kupvaWithRates);

        // Extract unique currencies (only from valid rates)
        const allCurrencies = kupvaWithRates
          .flatMap((kupva) => getValidExchangeRates(kupva.exchangeRates))
          .map((rate) => rate.mata_uang)
          .filter(Boolean) as Currency[];

        const uniqueCurrencies = allCurrencies.filter(
          (currency, index, self) =>
            index ===
            self.findIndex((c) => c && currency && c.id === currency.id)
        );
  ...
  const filteredKupvaData = kupvaData.filter((kupva) => {
    const validRates = getValidExchangeRates(kupva.exchangeRates);
    if (selectedCurrency === "all") return validRates.length > 0; // Only show KUPVA with valid rates
    return validRates.some((rate) => rate.mata_uang?.id === selectedCurrency);
  });
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 text-white">
            <div>
              ...
              <p className="text-white/80 mt-2">
                Money changers (KUPVA) terdekat di Provinsi Aceh
              </p>
            </div>
          </div>
        </div>
              ...
              >
                ...
                {availableCurrencies.map((currency) => (
                  <option key={currency.id} value={currency.id}>
                    {getCurrencyFlag(currency.kode)} {currency.nama} (
                    {currency.kode})
                  </option>
                ))}
              </select>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-12">
            <div className="flex flex-col items-center justify-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                Tidak ada KUPVA ditemukan
              </h3>
              <p className="text-gray-500">
                Belum ada KUPVA dengan kurs yang valid atau sesuai filter
              </p>
            </div>
          </div>
        ) : (
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
              <div className="p-4 border-b bg-gray-50">
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Klik marker untuk melihat detail lengkap
                </p>
              </div>
              <div
                  ...
                  {filteredKupvaData.map((kupva) => {
                    ...
                    return (
                      <Marker
                        key={kupva.id}
                        position={
                          [kupva.latitude, kupva.longitude] as LatLngTuple
                        }
                        eventHandlers={{
                          click: () => {
                                ...
                                <div className="space-y-2 max-h-32 overflow-y-auto">
                                      ...
                                      <div className="grid grid-cols-2 gap-2 text-xs">
                                        <div className="flex items-center gap-1">
                                          <span className="text-green-600 font-semibold">
                                            Rp {formatCurrency(rate.harga_beli)}
                                          </span>
                                        </div>
                                        <div className="flex items-center gap-1">
                                          <span className="text-red-600 font-semibold">
                                            Rp {formatCurrency(rate.harga_jual)}
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                  ...
                                  {validRates.length > 3 && (
                                    <p className="text-xs text-gray-500 text-center">
                                      +{validRates.length - 3} mata uang lainnya
                                    </p>
                                  )}
                                </div>
                              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-sm text-gray-600">
                  {filteredKupvaData.length} lokasi ditemukan
                </span>
              </div>
                ...
                {filteredKupvaData.map((kupva) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`absolute inset-x-0 bottom-0 bg-gradient-to-t from-white to-gray-50 border-t transition-all duration-300 ${
                          hoveredKupva === kupva.id
                            ...
                            <div className="space-y-2 max-h-32 overflow-y-auto">
                              ...
                              {validRates.length > 2 && (
                                <p className="text-xs text-gray-500 text-center py-1">
                                  +{validRates.length - 2} mata uang lainnya
                                </p>
                              )}
                            </div>
                          ) : (
                      ...
                      {/* Default Footer */}
                      <div
                        className={`p-4 border-t bg-gray-50 transition-all duration-300 ${
                          hoveredKupva === kupva.id
                            ? "opacity-0"
                            : "opacity-100"
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">
                            {validRates.length} mata uang tersedia
                          </span>
              ...
              {filteredKupvaData.length === 0 && (
                <div className="bg-white rounded-xl shadow-md p-12 text-center">
                  ...
                  <p className="text-gray-500">
                    Coba ubah filter mata uang atau periksa kembali nanti
                  </p>
                </div>
              )}
            </div>
