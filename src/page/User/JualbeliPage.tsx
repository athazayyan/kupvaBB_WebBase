...
export default function JualbeliPage() {
    ...
    .order('updated_at', { ascending: false });
                ...
                if (data) {
                    const formattedData = data.map(item => ({
                        ...item,
                        mata_uang: Array.isArray(item.mata_uang) && item.mata_uang.length > 0 
                            ? item.mata_uang[0] 
                    ...
                    setExchangeRates(formattedData as unknown as ExchangeRate[]);
                    setFilteredRates(formattedData as unknown as ExchangeRate[]);
                    
                    // Extract unique kupvas
                    const kupvas = [...new Set(formattedData.map(item => item.profile?.nama).filter(Boolean))];
    ...
    useEffect(() => {
        ...
        if (searchTerm) {
            filtered = filtered.filter(rate => 
                rate.mata_uang?.nama?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                rate.mata_uang?.kode?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        if (selectedKupva !== "all") {
            filtered = filtered.filter(rate => rate.profile?.nama === selectedKupva);
        }
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                ...
                                {uniqueKupvas.map(kupva => (
                                    <option key={kupva} value={kupva}>{kupva}</option>
                                ))}
                            </select>
                        </div>
                    </div>
                                ...
                                <table className="w-full">
                                    <thead style={{ backgroundColor: 'hsl(170.9, 37.5%, 34.5%)' }}>
                                        <tr className="text-white">
                                            ...
                                            <th className="px-6 py-4 text-right text-sm font-semibold">Harga Beli</th>
                                            <th className="px-6 py-4 text-right text-sm font-semibold">Harga Jual</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold">Terakhir Update</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold">Money Changer</th>
                                        </tr>
                                    </thead>
                                            ...
                                            filteredRates.map((rate) => (
                                                <tr key={rate.id} className="hover:bg-gray-50 transition-colors">
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-3">
                                                            <span className="text-2xl">{getCurrencyFlag(rate.mata_uang?.kode || '')}</span>
   ...
   {new Intl.NumberFormat('id-ID', {
                        ...
                        <div className="lg:hidden space-y-4">
                            {filteredRates.length > 0 ? (
                                filteredRates.map((rate) => (
                                    <div key={rate.id} className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow">
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                <span className="text-3xl">{getCurrencyFlag(rate.mata_uang?.kode || '')}</span>
                                                <div>
                                                    <h3 className="font-bold text-gray-900">{rate.mata_uang?.nama || 'N/A'}</h3>
